...
|---|---:|---|
| `p2p-netcat-core` | `0.2.1` | Browser-safe protocol, discovery, route planning, Trystero и PTY-примитивы |
| `p2p-netcat` | `3.0.2` | Node.js CLI и программная точка входа `p2p-netcat/relay` |
| `p2p-netcat-web` | `0.3.1` | Собранная статическая PWA в каталоге пакета `dist` |
...
`name@version` нельзя опубликовать снова никогда. Версия `3.0.2` намеренно выше
удалённых релизов.
