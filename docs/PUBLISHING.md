...
|---|---:|---|
| `p2p-netcat-core` | `0.2.1` | Browser-safe protocol, discovery, route-planning, Trystero, and PTY primitives |
| `p2p-netcat` | `3.0.2` | Node.js CLI and the `p2p-netcat/relay` programmatic entrypoint |
| `p2p-netcat-web` | `0.3.1` | Prebuilt static PWA in the package `dist` directory |
...
never be reused. Version `3.0.2` is intentionally newer than the deleted
releases.
...
npm install --prefix "${release_test_dir}" \
  p2p-netcat-core@0.2.1 \
  p2p-netcat@3.0.2 \
  p2p-netcat-web@0.3.1
