...
adapter публикует через несколько relay короткоживущие подписанные offer,
answer и trickle-ICE candidate events. Endpoint хранит ограниченный
...
3. libp2p stream ждёт `onDrain()`. WebRTC stream сначала согласует `flow:1`,
...
7. Во время восстанавливаемого WebRTC-разрыва те же ограниченные очереди ждут
   до 120 секунд: они не превращаются в EOF и не растут без ограничения.
