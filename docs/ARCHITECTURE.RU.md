...
trackers, экспортируемым установленным пакетом Trystero. Автоматическое
переподключение signaling-сокетов работает всё время жизни listener или попытки
подключения и приостанавливается только при намеренной остановке. После
аутентификации управляющий обмен `ping`/`pong` раз в 15 секунд поддерживает
неактивный Trystero data channel.
...
Интерактивный терминальный вывод проходит через ограниченный сквозной pipeline:

1. `node-pty` выдаёт терминальные байты, а сервер кодирует их как PTY data
   frames.
2. Серверный sender считает байты очереди. При 512 КиБ он вызывает
   `IPty.pause()`, а после уменьшения очереди до 128 КиБ — `IPty.resume()`.
3. libp2p stream ждёт `onDrain()`. Trystero stream сначала согласует `flow:1`,
   затем ограничивает неподтверждённый payload окном 256 КиБ.
4. Получатель отправляет `ack:<bytes>` только тогда, когда его async iterator
   переходит к следующему блоку после обработки предыдущего. Старый пир без
   `flow:1` продолжает работать по прежним правилам.
5. Для браузерного libp2p-маршрута Worker допускает не более 512 КиБ вывода,
   ожидающего main thread, и продолжает чтение после снижения до 128 КиБ.
6. Main thread подтверждает Worker-блок только после вызова callback,
   переданного в `Terminal.write()` xterm.

Поэтому память ограничена на каждой границе producer/consumer. Интерактивный
вывод намеренно не сохраняется в браузерный буфер скачиваемого ответа; историей
терминала служит конечный scrollback xterm. При нормальном закрытии удалённой
стороной ожидающие PTY-записи отбрасываются во время teardown без вводящей в
заблуждение вторичной ошибки `Cannot write to a stream that is closed`. Ошибка
записи при всё ещё открытом потоке остаётся настоящей ошибкой сеанса.
